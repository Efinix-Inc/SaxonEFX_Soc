...
import vexriscv.plugin.{BranchPlugin, CsrAccess, CsrPlugin, CsrPluginConfig, DBusCachedPlugin, DecoderSimplePlugin, DivPlugin, FullBarrelShifterPlugin, HazardSimplePlugin, IBusCachedPlugin, IntAluPlugin, MmuPlugin, MmuPortConfig, MulPlugin, RegFilePlugin, STATIC, SrcPlugin, StaticMemoryTranslatorPlugin, YamlPlugin, IBusSimplePlugin, DBusSimplePlugin}
...
object EfxRiscvBmbDdrSocParameter{
  def defaultArgs(args : Seq[String]): EfxRiscvBmbDdrSocParameter ={
    var cpuCached : Boolean = true
    var softTap : Boolean = false
    var axiAEnable : Boolean = false
    var iCacheSize = 4096
    var dCacheSize = 4096
    var onChipRamHexFile = "software/standalone/bootloader/build/bootloader.hex"
    var systemFrequency = 50000000
    ...
    assert(new scopt.OptionParser[Unit]("EfxRiscvAxiDdrSocGen") {
      ...
      opt[Boolean]("cpuCached")  action { (v, c) => cpuCached = v} text(s"Turn cpu cache on or off. Default $cpuCached")
      opt[Boolean]("softTap")  action { (v, c) => softTap = v} text(s"Configure debug tap type. Default $softTap")
      opt[Boolean]("axiAEnable")  action { (v, c) => axiAEnable = v} text(s"Turn axi4 on or off. Default $axiAEnable")
      ...
      opt[Int]("systemFrequency")action { (v, c) => systemFrequency = v } text(s"CPU + peripherals frequancy (set the UART baudrate at reset). Default $systemFrequency")
  ...
  def default(linuxReady : Boolean,
    ...
    val config = EfxRiscvBmbDdrSocParameter(
      systemFrequency = systemFrequency Hz,
        ...
        plugins = List(
          if(cpuCached == true) {
            new IBusCachedPlugin(
              ...
              //prediction = vexriscv.plugin.STATIC,
          ...
          if(!linuxReady){
            ...
            new CsrPlugin(CsrPluginConfig.linuxMinimal(null).copy(
              ebreakGen = false,
