...
class EfxJadeSocSystem() extends Generator{
...
class EfxJadeSocSystemWithArgs(p : EfxRiscvBmbDdrSocParameter) extends EfxJadeSocSystem{
  ...
  if( p.axiAEnable == true) {
    val axiA = new Generator {
      ...
      val logic = add task new Area {
        ...
        interrupt.setName("axiAInterrupt")
      }
    }
  }
...
class EfxJadeSoc(p : EfxRiscvBmbDdrSocParameter) extends Generator{
  ...
  val system = new EfxJadeSocSystemWithArgs(p)
...
object EfxJadeSoc {
  ...
  def main(args: Array[String]): Unit = {
    val report = SpinalRtlConfig.copy(
      ...
      if(p.softTap == true) {
        val toplevel = new EfxJadeSoc(p){
          system.cpu.enableJtag(debugCd, systemCd)

        }.toComponent("JadeSoc_softTap")
    ...
    BspGenerator("efinix/EfxJadeSoc", report.toplevel.generator, report.toplevel.generator.system.cpu.dBus)
  }
}
